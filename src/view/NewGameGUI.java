...
public class NewGameGUI extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		FootballManager.setDiv(null);
//		for(int i = 0; i < 18; i++){
//			FootballManager.getDiv().get(i).SetPcControlled(false);
//		}
		readXML test = new readXML();
		Division rest = new Division();
		test.runExample(rest, "src/model/XML5.xml");
		System.out.println(rest.get(0).getName() + rest.get(0).getBudget() + "Test res");
		FootballManager.setDiv(rest);
		System.out.println(FootballManager.getDiv().get(0).getName() + FootballManager.getDiv().get(0).getBudget() + "Test2");
